...
class FFTCalculator():
	def __init__(self, req: request.Request):
		...
		self.fft_files = list(str())
		self.fft_root = './cache/'
		self.path = ''


	def _find_valid_data_files(self, req: request.Request):
		...
		for file in Path('..').rglob(self.path + '**/*.cdf'):
			...
			if match:
				...
				if st <= ftime <= end:
					self.data_files.append(str(file))

		self.data_files.sort()

	def _cache_data_files(self, req: request.Request):
		pass
